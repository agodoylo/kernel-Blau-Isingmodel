...
def likelihood(KKH, aarea,seta,ppm,userw,JJ):
	like=0.
	l1=0.
	l2=0.
	for i in seta.keys():
...
def GgenerateSpins(KKH,userw,Kparams,ppm,aarea,ppair,JJ):
	#random network:
	spins=[-1,1]
	nseta={}
	users=len(userw.keys())
	...
	count=0
	L,gg1,gg2=likelihood(KKH, aarea,nseta,ppm,userw,JJ)
	i=0
	ext_time=0
	cacc=0
	T=1.
	llen=aarea.keys()
	while count<55*users:
...
def areadistance(K,KH,KH12,BKH,Kparams,aarea,Baarea,inBor,ppair,size,userw,J,J12,BJ,ggg):
	...
	if AA<3:	#We only generate spin cofigurations for networks with average degree (<K>) smalles than 6 (<K>=2L/N; AA=L/N)
	#same network for the three elections
		...
		res12=[dall12/float(tall12),di12/float(tutto12),d122/float(t122),(di12+d122)/float(tutto12+t122)]
		fout2.close()
...
##########################################################################
Nnumber=int(sys.argv[1])

fh2=open('EUwards.dat','r')
igot2=fh2.readlines()
fh2.close()
...
fh=open('ME16-12_sociodemographics.dat','r')
igot=fh.readlines()
del igot[0]
fh.close()
...
for line in igot:
	...
	w=about[0]
	area[w]=[]
	area[w].append([1.-float(about[2]),1.-float(about[3])])
	size[w]=float(about[10])
	vec=[]
	vec.append([float(about[4])])	#edu
	vec.append([float(about[5])]) #age
	vec.append([float(about[6])]) #gender
	vec.append([float(about[7]),float(about[8])]) #distance
	vec.append([float(about[9])])	#income
	area[w].append(vec)
	area[w].append(int(about[1]))
	#list of Boroughs missing in EUref data:
